...
def issue_book(request, book_id):
    ...
    if book.available_copies <= 0:
        messages.error(
            request,
            "Sorry, this book is out of stock and cannot be issued.")
    ...
    if issued_books_count >= issue_limit:
        messages.error(request,
                       ...
                       f"{issue_limit} issued books."
                       f"Please return it to issue a new one.")
        return redirect('book_list')
...
def return_book(request, issued_book_id):
    """
    View to handle the return of an issued book by
    the logged-in user.

    Parameters:
        issued_book_id (int): The ID of the issued
        book record to be returned.
    """
    issued_book = get_object_or_404(IssuedBook,
                                    id=issued_book_id,
                                    returned=False)
    ...
    if request.method == 'POST':
        if 'confirm' in request.POST:
            ...
            messages.success(request,
                             f'{issued_book.book} returned successfully!')
    ...
    return render(request,
                  'issues/return_book_confirm.html',
